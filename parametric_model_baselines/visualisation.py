...
def plot_parametric_data(df, data=False, max_only=True):
    ...
    fig, axs = plt.subplots(
        ncols=3, nrows=2, figsize=(9,6))


    x = np.arange(len(dfs['par']['hpso']))
    axs[0][0] = create_standard_barplot_axis(axs[0][0], False)
    axs[0][0].bar(
        x, dfs['par']['time'], color="lightblue", zorder=1,
    ...
    axs[0][1].bar(
    x-0.2, dfs['par']['time'], color="lightblue", zorder=1, edgecolor='black',
        width=0.4, log=True, alpha=0.5
    )
    axs[0][1].bar(
        x+0.2, dfs['scatter']['time'], color='pink', zorder=1,
        edgecolor='black',
        width=0.4, log=True
    )
    ...
    axs[0][2].bar(
    x-0.3, dfs['par']['time'], color="lightblue", zorder=1,
        edgecolor='black',
        width=0.3, log=True,alpha=0.5
    )
    axs[0][2].bar(
        x, dfs['scatter']['time'], color='pink', zorder=1,
        edgecolor='black',
        width=0.3, log=True,alpha=0.8
    )
    axs[0][2].bar(
        x+0.3, dfs['proto']['time'], color='orange', zorder=1,
        edgecolor='black',
        width=0.3, log=True
    )
