...
class BatchProcessing(Algorithm):
    ...
    def _max_resource_provision(self, cluster, max_ingest):
        """
        ...
        #   occur?
        max_allowed = int( (len(cluster.dmachine) - 5) / self.max_resources_split)
        if available == 0:
            return None
    ...
    def _provision_resources(self,cluster, workflow_plan):
        ...
        if cluster.get_idle_resources(workflow_plan.id):
            ...
            provision = self._max_resource_provision(cluster, workflow_plan.max_ingest)
            if provision < self.min_resource_per_workflow:
                logger.info('Waiting to provision resources')
                return False
            else:
                logger.info(f'Provisioning {provision} resources to {workflow_plan.id}')
